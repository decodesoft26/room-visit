  ...
  const visitsQuery = useQuery({
    ...
    queryFn: () =>
      api<{ visits: Visit[]; pagination: Pagination }>(
        `/api/visits?hotelId=${hotelId}&roomId=${roomId}&page=${page}&limit=10&search=${encodeURIComponent(debouncedSearch)}&status=${statusFilter}`
      ),
      ...
      <>
        ...
        <main className="min-h-svh p-5 pt-20 md:pt-28">
          <div className="mx-auto max-w-5xl py-20 text-center">
            <Building2 className="mx-auto h-16 w-16 text-[#7b918c]" />
            <p className="mt-4 text-lg text-[#67857d]">Loading room...</p>
          </div>
        </main>
      </>
    )
